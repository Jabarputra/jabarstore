...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------------------${NC}"
echo -e "  Welcome To Jabar Store Project Script Installer ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "         This Will Quick Setup VPN Server On Your Server"
echo -e "           Auther : ${green}JabarStoreVpn ${NC}${YELLOW}(${NC} ${green}Jabar Store Project ${NC}${YELLOW})${NC}"
echo -e "                 © Jabar Store Project ${YELLOW}(${NC} 2024 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
data_ip="https://raw.githubusercontent.com/Jabarputra/jabarstore/main/izin/ip"
...
checking_sc() {
  ...
  if [[ $(date -d "$date_list" +%s) -lt $(date -d "$useexp" +%s) ]]; then
    ...
    echo -e "\033[42m       JABAR STORE AUTOSCRIPT PREMIUM       \033[0m"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    echo -e ""
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/Jabarputra/jabarstore/main/izin/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/Jabarputra/jabarstore/main/izin/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/Jabarputra/jabarstore/main/izin/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/Jabarputra/jabarstore/main/"
...
function notif_bot() {
    ...
    USRSC=$(curl https://raw.githubusercontent.com/Jabarputra/jabarstore/main/izin/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(curl https://raw.githubusercontent.com/Jabarputra/jabarstore/main/izin/ip | grep $ipsaya | awk '{print $3}')
...
<i>JabarVpn</i>
"'&reply_markup={"inline_keyboard":[[{"text":"ORDER SC","url":"https://t.me/jabar_putra"},{"text":"CHANNEL","url":"https://t.me/jabar_vpn"}]]}'
...
wget ${REPO}レスキセティワン/limit.sh && chmod +x limit.sh && ./limit.sh
